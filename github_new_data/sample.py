...
def main():
    files = []
    with open('TopLists/Python-top-repos.txt', 'r') as fr:
        for line in fr.readlines():
            ...
            data_dir = f'Code/Python/{github_org}/{github_repo}'
            if not os.path.exists(data_dir):
                continue
            for file_name in os.listdir(data_dir):
                if os.path.isfile(os.path.join(data_dir, file_name)):
                    files.append(os.path.join(data_dir, file_name))
    print(f"Obtained {len(files)} deduplicated Python files from GitHub.")
    
    random.seed(42)
    random.shuffle(files)
    
    tokenizer = AutoTokenizer.from_pretrained("codeparrot/codeparrot")
    target_csv_file = '../unlearning/data/github/github_new_data.csv'
            ...
            try:
                with open(file_path, 'r', encoding='utf-8') as data_fr:
                    ...
                    if length > 200 and length < 1000:
                        corpus = file_path.replace('Code/Python', 'GitHub')
