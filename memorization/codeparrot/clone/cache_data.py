...
def download_and_save_data(data_name, cache_dir=None, data_dir=None, split='train', save_dir='./save'):
    ...
    num_of_batches = int(np.ceil(len(ds) / batch_size))
    for i in tqdm(range(num_of_batches)):
        with open(os.path.join(save_dir, str(i)), 'w') as f:
            for j in range(i * batch_size, (i + 1) * batch_size if (i + 1) * batch_size < len(ds) else len(ds)):
...
if __name__ == '__main__':
    ...
    download_and_save_data(
        data_name="codeparrot/codeparrot-clean-train",
        cache_dir=cache_dir,
        split="train",
        save_dir=save_dir
    )
